...
interface Operator {
  id: any; name: any; email: any; qualification: any; active: any;
}

export default function OperatorPage() {
  const [items, setItems] = React.useState<Operator[]>([]);
  const [showForm, setShowForm] = React.useState(false);
  const [form, setForm] = React.useState<any>({ name: '', email: '', qualification: '', active: '' });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setForm({ name: '', email: '', qualification: '', active: '' });
    setShowForm(false);
    load();
  };
  ...
  <input name="qualification" placeholder="qualification" type="text" value={form.qualification || ""} onChange={e => setForm({...form, qualification: e.target.value})} className="border rounded px-3 py-2 text-sm" />
  <label className="flex items-center gap-2"><input type="checkbox" name="active" checked={!!form.active} onChange={e => setForm({...form, active: e.target.checked})} /> active</label>
      ...
      <div className="bg-white rounded-xl border overflow-x-auto">
        <table className="w-full">
          <thead className="border-b"><tr><th className="text-left p-2 text-xs text-gray-500 uppercase">id</th> <th className="text-left p-2 text-xs text-gray-500 uppercase">name</th> <th className="text-left p-2 text-xs text-gray-500 uppercase">email</th> <th className="text-left p-2 text-xs text-gray-500 uppercase">qualification</th> <th className="text-left p-2 text-xs text-gray-500 uppercase">active</th><th></th></tr></thead>
          <tbody>
            {items.map((item) => (
              <tr key={item.id} className="border-b last:border-0 hover:bg-gray-50">
                <td className="p-2 text-sm">{String(item.id ?? "")}</td> <td className="p-2 text-sm">{String(item.name ?? "")}</td> <td className="p-2 text-sm">{String(item.email ?? "")}</td> <td className="p-2 text-sm">{String(item.qualification ?? "")}</td> <td className="p-2 text-sm">{String(item.active ?? "")}</td>
                <td className="p-2">
