...
from dsl2doql.events import default_event_store
from dsl2doql.grammar import parse_line, split_command, to_text
from dsl2doql.handlers.command import handle_from_tokens
from dsl2doql.result import DslResult
from dsl2doql.schema_registry import validate_command_dict

QUERY_VERBS = frozenset({"QUERY", "RESOLVE", "VALIDATE"})
COMMAND_VERBS = frozenset({"PATCH", "UPDATE", "REPLACE", "APPEND", "APPLY", "GENERATE", "CONVERT", "ADOPT", "MATERIALIZE"})
...
def _bytes_to_cmd(data: bytes) -> tuple[dict[str, Any], str]:
    from dsl2doql.pb_codec import decode_protobuf
