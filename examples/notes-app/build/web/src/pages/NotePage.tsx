...
interface Note {
  id: any; notebook: any; title: any; body: any; pinned: any; created: any; updated: any;
}

export default function NotePage() {
  const [items, setItems] = React.useState<Note[]>([]);
  const [showForm, setShowForm] = React.useState(false);
  const [form, setForm] = React.useState<any>({ notebook: '', title: '', body: '', pinned: '' });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setForm({ notebook: '', title: '', body: '', pinned: '' });
    setShowForm(false);
    load();
  };
          ...
          <tbody>
            {items.map((item) => (
              <tr key={item.id} className="border-b last:border-0 hover:bg-gray-50">
                <td className="p-2 text-sm">{String(item.id ?? "")}</td> <td className="p-2 text-sm">{String(item.notebook ?? "")}</td> <td className="p-2 text-sm">{String(item.title ?? "")}</td> <td className="p-2 text-sm">{String(item.body ?? "")}</td> <td className="p-2 text-sm">{String(item.pinned ?? "")}</td> <td className="p-2 text-sm">{String(item.created ?? "")}</td> <td className="p-2 text-sm">{String(item.updated ?? "")}</td>
                <td className="p-2">
