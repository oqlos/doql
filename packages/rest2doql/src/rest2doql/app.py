...
from dsl2doql import dispatch
from dsl2doql.events import default_event_store
from dsl2doql.pb_codec import encode_result_protobuf
from dsl2doql.schema_registry import all_schemas, schema_for_verb

from fastapi import FastAPI, Request
from fastapi.responses import JSONResponse, PlainTextResponse, Response

_PROTO_DIR = Path(__file__).resolve().parents[3] / "dsl2doql" / "proto" / "dsl2doql" / "v1"


def create_app() -> FastAPI:
    app = FastAPI(title="rest2doql", version="0.1.0")

    @app.get("/health")
    def health() -> dict[str, str]:
        return {"status": "ok", "service": "rest2doql"}
    ...
    def list_proto() -> JSONResponse:
        ...
        return JSONResponse({"package": "dsl2doql.v1", "files": files})
