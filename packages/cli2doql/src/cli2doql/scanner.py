...
from doql.parsers.models import DoqlSpec, Interface, Page

def is_server_entry(name: str, target: str) -> bool:
    lowered = f"{name} {target}".lower()
    return "server" in lowered or (".api:" in lowered and "api" in name)


def is_mcp_entry(name: str, target: str) -> bool:
    lowered = f"{name} {target}".lower()
...
def scan_python_cli(root: Path, spec: DoqlSpec) -> None:
    ...
    for name, ep in scripts.items():
        if is_server_entry(name, ep) or is_mcp_entry(name, ep):
            continue
